...
final public class MatrixPokemonTransformer implements PokemonTransformer<String[][]> {

    private final String[] COLUMNS = {"ID", "Name", "Type 1", "Type 2", "HP", "Atk", "Def", "SpA", "SpD", "Spe"};

    private final MatrixService matrixService;

    public MatrixPokemonTransformer(MatrixService matrixService) {
    ...
    public String[][] transform(PokemonCollection collection) {
        ...
        for (int i = 0; i < pokemons.size(); i++) {
            ...
            matrix[i+1][4] = String.valueOf(pokemons.get(i).baseStats().healthPoints());
